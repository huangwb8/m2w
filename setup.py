...
+ 使用"python setup.py sdist"命令来生成项目的源代码包。 如果要测试该包，可运行类似命令： python setup.py sdist; pip install .\dist\m2w-2.5.8.tar.gz

+ 使用"python setup.py bdist_wheel"命令来生成项目的长描述。

+ 先完成Github仓库的更新和加tag。最后再上传至PyPi。输入"twine upload dist/*2.5.8* --verbose"来上传项目的源代码包。
